...
本规格按 `to-spec` 汇总已定案的目标，不新增产品决策，也不把历史研究或 prototype 的结果升级为目标实现证据。`ready-for-agent` 仅为本地 tracker 的规格标签，不是执行指令；整体评审、独立仓库迁移和各阶段门禁未通过前，不启动生产实现或 MC cutover。原始契约及其责任边界见 Further Notes；发生冲突时显式阻断并回到对应 owner 裁决，不由本规格静默覆盖。
...
v1 聚焦 memory lifecycle 和 context 协作。AGENTS、Skill、Core tool 与 MCP 是受验证的指令/能力接入面，不拥有 canonical memory。只自动激活经门禁的 Memory 与可重建 retrieval index；行为修改仅保存/导出带 evaluation contract 的 inert proposal。
...
高频过滤和不变量用类型化列、约束与外键；JSON 仅用于版本化校验后、不参与核心门禁的 metadata。最小表族包含 identity/control（含 `intent_events`/`intent_heads`）、memory revision/event/head、ownership/evidence、显式 owner 的 execution streams、projection outbox/leases、scope-review input watermark/cursor、scope-only `projection_artifacts`/typed input refs、search、Host presentation/pending，以及 content-free purge receipts 和 `owner_fences`/`owner_activities`。真实 owner mutation receipt 与 canonical 变更同事务追加在 memory events；Host operation state 不是第二份 receipt 真值，也不写入 execution events；已结算 purge 的最小 cleanup continuation 复用 pending_operations，不等于等待批准的 pending，完成前按隐私边界清除其敏感字段。Storage-owned identity、append-only 与完整 payload digest 由存储层强制：调用方不得原地改写 canonical row/revision/event、伪造行 identity 或只对 ID 重算 receipt/manifest digest；10 §28b 的数据库后置条件失败时事务回滚且外部删除次数必须为 0。

全部表族、不变量、Host receipt、backup/purge 与失败恢复门槛通过前，只用合成可删除数据库。初始 schema 正式冻结产物含 DDL、版本、SHA-256 和实现 commit；冻结前可重建，不写原型兼容迁移；冻结后初始迁移不可改写，只前进迁移，事前恢复点、失败保留旧库、未知更高版本拒开。P0 准备或 P1 通过均不单独授权真实 memory/ledger 写入。
...
Proposer/Historian 推荐标准级，Historian 还需长上下文和忠实压缩；Verifier 按风险选能力，低风险允许同模型隔离上下文，高影响/跨 scope/安全候选增加不同模型、真实工具/平台、held-out/canary 或人工。Provider/model 可配置，无型号白名单；低于所需能力只能停留 candidate/evidence-gap。模型能力、reasoning effort 与编排拓扑不等同。行为制品 proposal 必须在 `evolution_proposals` 内绑定两个版本化对象：评估前 `draft → sealed` 的 `evaluation_plan`（target artifact/type、scope、风险级别与 required L0–L3、source/input refs 与授权边界、assertions、baseline/checkpoint/replay policy、held-out/越界负例或 canary 要求、verifier independence、budget/stop/success-failure rules、owner/sealed_at、canonical plan digest）和评估后追加的 `evaluation_result`（storage-owned result identity、plan id/digest、实现 commit/fixture digest、attempt set、独立 verifier identity、结果与 failure state）。evaluation start 必须在同一 durable barrier 中确认 plan 已 sealed、digest/identity/fence 匹配后再登记 attempt；result append-only，plan/result 行不可 UPDATE。Core/Store 读取并强制 typed fields、完整 canonical bytes/digest、owner seal、superseding identity 与 purge reverse refs；任何 target、scope、risk、required level、source/input、assertion、baseline/checkpoint/replay、held-out/negative/canary、verifier/budget/stop/success-failure 字段变化都必须创建新 proposal/plan/result lineage，不能由 superseding result 降低门槛或替换评估集。plan/result 缺失、结构无效、不可归因、seal 顺序未知或证据不足时只能保持 inert，不得获得行为权限。
...
source-ingress handoff 必须先由 source owner 提供可重放 cursor，再由 Core 同事务登记 `input_seq`/outbox 后返回 handoff ack；无法重建时为 `evidence-gap`。scope review 的 store/per-scope active drain cap、overview file publish protocol 和 raw coverage watermarks 由 08/10/12 的细化契约共同约束。

跨会话整体分析不依赖每轮重读全部 archive。现有 maintenance/projection worker 另以 `scope-review` job 按 logical project、workspace 或 personal scope 维护 cursor/generation：输入是新增或变化的 canonical memory/event、同 subject/resource/scope/time 的受影响邻域、当前 overview 引用和确定性 gate 标出的冲突/过期/provenance 缺口。确定性层先产出 supersession、conflict、scope drift、duplicate、missing evidence 等关系；需要语义判断时，现有 Proposer/Historian/Verifier 在有界 working set 上生成带 source refs 的 candidate 或 overview patch，不能把同义摘要直接晋升为新 memory。首次导入、cursor gap、recovery gap 或明确全局重审才走可暂停的分批 backfill。

`scope_overview` 是可重建的 app-data 派生 projection，不是真实 memory、instruction、policy 或权限；它只总结当前 verified/active 决策、适用约束、变化、未决问题和 evidence-gap，并绑定 input generation、source/memory refs digest、逐条 source owner scope/project/resource/`applies_to`、membership/`derived_from` snapshot digest、content hash、scope、generator 和 verifier 状态。Context Orchestrator 只在 scope 匹配且 projection fresh、可验证时把它作为有界 baseline；stale/conflicted/purged/cursor-gap/review-failed/repair_required 时标记不可注入并走局部 retrieval/source recovery。review job 失败不修改 canonical memory；purge 清除 overview 正文、refs、locator/hash 和 cursor 关联。
...
最窄 scope 先满足最低充分证据再激活。v1 实现明确 owner/结构化事实的快速通道与普通 project 通道；高风险与跨 scope 保持额外门禁，完整自动广域 canary 不是首次可用前置。候选必须有阶段、原因和重试/升级路径，不能无声 pending。`evaluation_contract` 的 L0–L3 选择按 target 与风险取最低充分层级：L0 检查 source/hash/scope/conflict/schema，L1 检查触发/注入/首个关键选择，L2 复用最近 decision checkpoint 做 baseline/treatment，L3 才要求独立 held-out、越界负例或 canary；Skill、AGENTS、policy 等行为制品仍须附各自适用、不适用、边界、安全和成本断言。缺失或失败时保持 inert/blocked/rejected，不由 fallback 改成通过。
...
Purge 先展示删除闭包、受控范围、外部残留与不可恢复后果，再核对 Host 铸造、绑定 token/target-or-batch 的精确 nonce 口令。只接受同一条真实活输入事件中的完整连续字节匹配，先经过冲突/取消门禁，不接受泛化“确认”、历史口令或 extension 注入；批量一次批准覆盖冻结 manifest。
...
批准后按 10 §23a 的 `owner_fences`/`owner_activities` 协议关闭 admission、排空旧活动，再重验完整 CAS/manifest 与 owner/resource identity。登记 activity 与检查 fence open 必须同一个短事务，总是同时覆盖 store 与实际外部 root；所有内容读取/缓存/写入/子任务/发布/backup/恢复都保持登记直到无迟到输出能力。closing 增 epoch 后新普通活动禁止；唯一受限 purge-control 协调者可重验/重显 manifest、接收批准/取消及保存进度，不等待自身旧 activity，不得调用模型或扩大外部删除；其敏感显示/控制记录随同 operation 自清除。接管须证实旧协调者停止，不靠超时并行删除。旧进程 TTL/心跳丢失不算已终止，必须有 quiesce ack 或真实宿主的进程及子任务退出/残留证明。排空后 manifest 增变须重新展示批准，不能扩大原授权。已承诺受控者失联保持 blocked，只有事先声明不能受控者列外部残留。

Pi 接线遵循 13 的 P0 release 条件：session/window activity 从首次 source 读取前覆盖所有内容及输出能力，run/transport 结束不留下未登记的缓存空档。公开 append 先重验 fence，再完成其 durable ack；底层 Agent listener 的 await 与 session.subscribe 的非 await 必须区分并逐实际 Promise/callback join。agent_end/settled/idle/dispose/清空可见 editor 不单独构成 quiesce，undo/history/next-turn/provider/TUI 队列均须失效；无法证明时沿用真实进程及子任务退出和残留检查。不得在被 await 的 handler 内等待同一 run 自锁；按 X-12 验迟到 append/回调、缓存复活负例及正常完成，未齐证据不签 ack。
...
Bundle 为 versioned manifest、events 与内容寻址 blobs。无显式 scope/project/session/record selector 拒绝导出；只展开所选 canonical/provenance/tombstone 必要闭包，owning session 被选才包含其 ledger；v1 不导出 job/maintenance/migration streams、active intent、fence/activity 或待执行批准，heads/index/jobs/leases 不导出。逐文件 hash、origin/event identity 与幂等检查，冲突保留，导入始终为待重验候选。Hash 不等于 authenticity，v1 不加签名/密钥体系或全局 hash chain；课程 X-08 的独立 owner pre-image 不改变生产密码学范围。

### I20. MC 替代、平台与阶段

文档迁入独立产品仓库与运行时替代 MC 是两件事。前者须整体评审通过并按 Further Notes 清单执行；后者只在 D7 与 X-card 实证门槛后发生。
...
先例为课程 context/memory/tool-gate 的确定性 fixture、Ticket 11 的可点语义原型及固定源码的反例。它们只用于发现遗漏，不直接复制成生产实现或证明目标行为。代码位置与证据清单在 Further Notes，原矩阵每张卡的字段、case、补充验收和失败影响全部生效，以下摘要不能缩减它们；原型/旧评审仅是历史基线，不证明本轮 head 事件 CAS 或分阶段 purge。

### X-card 覆盖与阶段

| ID | 阶段 | 必须观察的结果与完整 PASS 边界 |
|---|---|---|
| X-01 | P1 | app-id/数据根、schema/事务/FK/CAS/seq/head 重建及 session pending 唯一约束；同时验证 typed evaluation_plan/result、sealed-before-start barrier、append-only result、plan/result binding、FK/reverse-ref 与完整 digest；portable 与真实宿主分别记录。 |
| X-02 | P2 | 完整 eligibility、target FTS/CJK、current 去重与 held-out；先登记原 60 用例四模式套件 locator/case IDs/digest，缺件不得开跑。 |
| X-03 | P1 | 实际 worker 的乱序/重复/崩溃/租约接管、FTS 损坏重建、双向完整性；scope artifact 的 publish operation owner/fence、orphan GC 和 repair continuation 也必须通过；不能只验 DDL，未来生成式 artifact 不计入 v1 PASS。 |
| X-04 | P2，P3 接线复验 | 硬预算、四级降级、完整 ReAct、先归档后卸载、source recovery、单主 LLM 快路径、后台失败不阻塞；真实 Host dispatch 再验。 |
| X-05 | P2，P3 补齐 | intent/resource/manifest/path 下 active project 集合与 scope/provider 数据隔离；原文/metadata 不能改变 policy、工具、权限或凭据。 |
| X-06 | P1，P3 接线复验 | assembly/started 两道 barrier 的各杀点、最终 payload、网络计数、unknown-sent 与 recovery gap；stub 不能免除实际接线检查。 |
| X-07 | P2 | capture/Verifier 独立取源、时间/身份归一、冲突、强信号回滚、pending 原因与 retrieved/injected 区分；同时验证 evolution proposal 的 sealed plan/result Core admission、required level/checkpoint/held-out/source/budget/stop 条件不可改变，缺失/错误绑定保持 inert。 |
...
| X-10 | P3 | 自研 CLI/TUI、Pi TUI、合规 RPC、合规 JSON、Pi -p 的呈现/批准/终态与 receipt 两面；完整 PASS 必须同时含 X-11。 |
| X-11 | P3 | durable pending、JSON 跨进程、首轮未 flush、unknown commit outcome、仅补呈现不重做 mutation；与 X-10 联合放行。 |
| X-12 | P4 首项 | ENOSPC/只读/损坏/迁移中断、backup 原子保留、恢复 gap、main/WAL/free pages/备份/所有受控引用清除。 |
...
| X-14 | P4，X-15 后 | provider reported usage/cache、payload/assembly/epoch 身份与 cache on/off 安全等价；真实测量接 D7 baseline/阈值，不能反推 cache 命中。 |
| X-15 | P4，X-12 后 | 三 OS export/import 环路、显式 selector、完整依赖闭包、hash/幂等/origin/conflict/purge 排除；runner 可验 portable，真实 Host 另验。 |

P1 主链 `X-01→X-03→X-06`；P3 先 X-10 正常路径、再 X-11 故障点，逐 adapter/目标平台联合放行；P4 主链 `X-12→X-15→X-14`。X-09/X-13 的 P2 子门禁不要求尚未实施的 Host 全链已通过，也不允许据此提前开放相应能力。X-09 的受控真实模型检查不拖到 X-14，不以 mock 替代。
...
- D6 AGENTS scope/conflict、Skill 发现/同名/准确正文/换窗及多已选 Skill 的 skill-conflict、Core/MCP schema/binding/namespace/不可用：X-04/X-05/X-10，明确冲突输入、相关副作用为 0、相容及解除后正常对照；实际 adapter 接线重验，不要求任意自然语言冲突解析器。
...
- 本轮八项契约闭包按 Ticket 12 的具体 fixture/观测/PASS：intent X-01/04/05/11；stream owner X-01/06/07/12/15；Info batch/outbox/ack X-01/07/10/11；tombstone/suppression X-07/09/12；purge fence X-01/06/12；adapter-owned MCP discovery X-05/10/14；Pi 启动前加载集合、原生 compaction/retry/非模型入口与最终 transport X-04/05/06/10。P0 冻结 API/载体和真实 Pi 接点探针，P1 完成持久化，再验 P2/P3 行为；D7 prerequisite 不能绕过它们。

- Pi 公开接点闭包：同一 JSONL 首轮 ack/严格恢复 X-04/X-11，factory 后 callback/action 组合和各模式非模型入口 X-10，持续 session/window activity、实际 join/缓存失效及进程终止证明 X-12；具体 fixture/观测/成功与拒绝断言按 Ticket 12。P0 synthetic 原生接点探针只支持机制可行性，完整 Core/Host 和物理验收按 D8 后续阶段，不能因某公开 API 存在就提前放行。
...
整体评审通过后，迁移以下最小集合；这是代码库文档迁移，不是 MC 数据迁移：
...
原 F01-F04 已由 owner 确认最小修正并回写，等待对当前新 SHA 的完整独立复验，不把本表当 PASS：
...
| 事项 | 唯一详细条款 | 验收焦点 |
|---|---|---|
| purge 跨进程隔离 | 10 §23a | 原子 admission、排空而非 lease 过期、manifest 重批、store 末段恢复 |
...
完整复验还须覆盖增量验收闭包、HostAdapter source 契约、13 的 Pi 公开 API 候选/失败条件与迁移定位转换，并从全部原文重新寻找其他反例；不得只对照这些表或沿用旧 reviewer 结论。
