...
11. Memory Retriever 保持为本地、确定性、可单独测试的 retrieval primitive：先按 logical project/workspace/personal、`applies_to`、lifecycle 与 verification 做硬过滤，再对当前请求 + active intent 执行 sparse lexical 与 dense semantic 候选召回，最后做简单结果融合、claim 去重和 token-budget admission；scope 更窄、类型固定或时间更新本身都不构成万能排序优先级。
