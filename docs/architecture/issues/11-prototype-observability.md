...
2026-09-09 修订：首次切片只要求自研交互 CLI 的正常/恢复闭环；Pi 等后续模式在启用前复用相同语义。Privacy purge 首版只在 10 §23a 的维护模式执行：普通运行中可解释/转交请求，但不先批准删除；旧进程及子任务退出、取得维护独占后，维护 Host 再完整展示并取得一次真实批准。以下 purge 语义均发生在此条件内，不要求在线 UI/cache quiesce。历史原型不修改为生产实现，也不证明本次修订通过。
