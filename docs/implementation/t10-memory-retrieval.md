...
`ProbeStore.searchMemories(activity, request)` defaults to the bound local project plus applicable workspace/personal memory. `noActiveProject: true` removes that default; it never grants another project. A trusted synthetic Host must archive a separate `memory-discovery-approval@1` owner decision binding the active intent/goal event, exact registered project IDs, target environments and cumulative result/byte budgets. `authorizeMemoryDiscovery` re-reads that durable source and rejects any mismatched argument. The grant is rechecked on every query; completing the task or revoking the grant invalidates it and its cursors. Query-supplied target metadata cannot override a cross-project target fixed by the grant. No operation or guidance scope changes.
...
The evidence script writes raw observations to `artifacts/t10-*/summary.json` and focused test output to `focused-test.txt` (ignored by Git): implementation commit/worktree state, Node/platform/SQLite, source fixture digest, held-out case-level gold/results, and a two-page cross-project grant/revocation trace. It checks all frozen X01-X09 judgments against the focused test output. Run again after committing to bind a clean implementation SHA. It does not read MC or create production data.
