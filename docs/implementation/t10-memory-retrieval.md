...
`ProbeStore.searchMemories(activity, request)` defaults to the bound local project plus applicable workspace/personal memory. `noActiveProject: true` removes that default; it never grants another project. A trusted synthetic Host must archive a separate `user`-role `memory-discovery-approval@1` decision binding the active intent/goal event, exact registered project IDs, target environments and cumulative result/byte budgets. `authorizeMemoryDiscovery` re-reads the durable source and its Host-reported role, rejecting `assistant`/`tool`/unknown roles and mismatched arguments. Each intent admits **one** grant: replaying the same decision returns its original grant and counters, while a different decision cannot reset its budget or change its scope. A fixed additional query allowance of `min(128, max(8, maxResults * 4))` is durable and charges even empty or dirty searches. The grant is rechecked on every query; completion revokes it transactionally and permanently blocks new discovery for that intent, even if its status later returns to active. Explicit owner revocation works while paused. Query-supplied target metadata cannot override a cross-project target fixed by the grant. No operation or guidance scope changes.

Search checks projection/current-head agreement and FTS external-content integrity, then queries bounded FTS candidates and replays their canonical event, revision, verification evidence and source. Latin/digits use NFKC lowercase; Han runs add overlapping bigrams in the projection (`latin-cjk@3`), with script boundaries and canonical text preserved. Each literal query group forms a lexical lane; within-lane order is BM25, followed by deterministic round-robin selection and current-claim/environment/effective-validity dedup. Numeric query terms must match exactly before a candidate enters any lane (for example, `RFC 42` cannot admit `RFC 43`); separate numeric questions need separate searches. A one-Han-character query is unsupported; single-digit exact terms are supported. Applicability grammar is checked at capture and rechecked fail-closed on read; legacy bare agent/platform tokens remain accepted. No vectors, router/reranker LLM or fixed-result padding. Results carry project, source, version and target labels. Unknown cross-project targets are `reference_only`; expired/conflicted/stale states return only a bounded status with a claim digest. Dirty/unavailable responses never imply absence. Oversized candidates are skipped with an explicit partial-coverage reason; `complete: false` means the result is not a full-history analysis. The integrity check currently runs per query in the disposable probe; production performance and a cheaper guarded watermark require a measured workload.
