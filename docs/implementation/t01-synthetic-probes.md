...
| Probe receipt | `local-dispatch-probe@1`：run/session owner、assembly、encoding/adapter、最终 payload hash/bytes、count/outcome、`formalLedger=false`。完整 payload 只在原始 transport 观察中提供；receipt 不进入 payload。不是 X-06 两道 barrier 或持久 attempt ledger。 |
