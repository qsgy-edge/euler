...
- `recognize` accepts explicitly identified constraints with exact quote/hash/target/operation evidence. It adds evidence; it cannot retract an already reported contradiction. It does not parse arbitrary natural language. Ordinary AGENTS precedence is applied per target/key, retaining wider unrelated rules. Same-operation incompatible constraints produce `instruction-conflict` or `skill-conflict`; compatible rules and separate operations remain executable.
...
- `execute` revalidates current sources, intent, scope, authorization and policy/capability/credential/approval flags before a synchronous synthetic effect. It returns `not_started + unavailable` on refusal, `started + success` on completion, and `started + unknown` if an effect throws. These test grants and callbacks are not a new production tool executor or permission transport.
