...
`host_presentations` freezes a complete canonical snapshot, token, session/branch, epoch/order, originating inspect, replaced operation identity and any selected batch. `presentation_targets` supplies typed record/revision/head/scope and selected-event references. Output acknowledgement can only advance and cannot change the frozen payload. `pending_operations` has a partial unique index on session identity while pending; a new preview settles the old operation and names it in its own presentation. Other sessions keep their pending slots. Commit compares the complete frozen snapshots, including `head_event_id`, in the mutation transaction. Stale batches return the full available current target set and never silently shrink selection.
...
The maintenance inventory now recognizes every registered session carrier and blocks on a missing or replaced carrier, including one belonging to another session. This does not enable physical deletion, backup or an online cleanup mechanism.
